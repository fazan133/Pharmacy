...
import { useCompanies, useCreateCompany, useUpdateCompany, useDeleteCompany } from '../hooks/useMasters';
import { SimpleMasterFormDialog } from '../components/SimpleMasterFormDialog';
import { companyApi } from '../api/mastersApi';
...
export function CompaniesPage() {
  ...
  const { data, isLoading, error, refetch } = useCompanies({ includeHidden: true });
  ...
  const handleSave = async (formData: Partial<Company>) => {
    if (selected) {
      ...
      await createMutation.mutateAsync(formData as Company);
    }
    setFormOpen(false);
    setSelected(null);
  };
  ...
  return (
    <Box>
      <PageHeader
        title="Brands"
        subtitle="Manage product brands/manufacturers"
      ...
      <SimpleMasterFormDialog
        ...
        fields={fields}
        onGenerateCode={companyApi.generateCode}
      />
